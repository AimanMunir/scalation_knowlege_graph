...
import scala.collection.{SortedMapFactoryDefaults, StrictOptimizedIterableOps, StrictOptimizedMapOps, StrictOptimizedSortedMapOps}
...
/* for 2.13
      ...
      with StrictOptimizedIterableOps [(K, V), Iterable, BpTreeMap [K, V]]
      with StrictOptimizedMapOps[K, V, Map, BpTreeMap [K, V]]
      with StrictOptimizedSortedMapOps[K, V, BpTreeMap, BpTreeMap [K, V]]
...
/* for 2.12
      extends AbstractMap [K, V]
      with Map [K, V]
      with MapLike [K, V, BpTreeMap [K, V]]
      with Serializable
*/
{
     ...
     */
    def put (kv: (K, V)): BpTreeMap.this.type = { keyCount += 1; insert (kv._1, kv._2, root); this }
...
//  def -= (key: K): BpTreeMap.this.type = throw new NoSuchMethodException ("-= method not yet supported")
     ...
     */
//  override def compare (key1: K, key2: K): Int = key1 compare key2
