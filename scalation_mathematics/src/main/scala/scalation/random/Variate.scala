 ...
 *  @param stream      the random number stream
 */
case class Discrete (dist: Array [Double] = Array (.2, .2, .2, .2, .2), x:  Array [Double] = null,
                     cumulative: Boolean = false, stream: Int = 0)
     extends Variate (stream)
{
    private val cdf = if (cumulative) dist else cumulate (dist)
    private val n   = dist.length
    private val xx = if (x == null || x.length == 0) Array.range (0, dist.length).map (_.toDouble)
                     else x
     ...
     extends Variate (stream)
{
    ...
    val x = Array.range (0, n).map (_.toDouble)   // outcomes range 0 until n
    var t = 0.0                                   // time
    ...
    private def sum (w: Array [Double]): Double =
    {
       var s = 0.0
...
//  import scalation.plot.Plot
    ...
    val dice  = Randi (1, 6)
    val x     = Array.range (0, 7)
    val freq  = Array.ofDim [Double] (7)
    val x2    = Array.range (0, 13)
    val freq2 = Array.ofDim [Double] (13)
    for (i <- 0 until 10000) {
        val sum  = dice.igen
...
//  import scalation.stat.Histogram
    ...
    val rvg = Uniform ()
    val x = (for (i <- 0 until 100000) yield rvg.gen + rvg.gen + rvg.gen + rvg.gen).toArray
//  new Histogram (x)
