 ...
 *  @param stream  the random number stream
 */
case class NHPoissonProcess (lambda: Array [Double], dt: Double = 1.0, stream: Int = 0)
     extends TimeVariate (stream)
{
    if (lambda.exists (_ < 0.0)) flaw ("constructor", "parameter vector lambda must be nonnegative")

    private val lsum    = cumulate (lambda).map (_ * dt)  // cumulative lambda
    ...
    {
        if (a < 0.0 || b > lambda.length * dt) { flaw ("pf", "time bounds are outside total time interval"); -1.0 }
        else {
            val aRes = a % dt
            val bRes = b % dt
    ...
    def genTime: Double =
    {
        ...
        for (i <- 0 until lsum.length if e <= lsum(i)) {      // find where lsum(i-1) < e <= lsum(i)
...
object PoissonProcessTest extends App
{
    ...
    val lambda = Array [Double] (2, 2, 2, 2, 2, 2, 2, 2, 2, 2,
                                 4, 4, 4, 4, 4, 4, 4, 4, 4, 4)

    val pp   = PoissonProcess (lambda.sum / lambda.length.toDouble)
