...
class GraphqlOpt (g: Graph, q: Graph) 
{
     ...
     private val qProfileMap   = HashMap [Int, ListBuffer [TLabel]] ()   // profile mapping of each query vertex 'u' to empty list
     private val gProfileMap   = HashMap [Int, ListBuffer [TLabel]] ()   // profile mapping of each data vertex 'v' to empty list
...
//   } // refineSearchSpace
      ...
      */
     def getNeighborProfile (graph: Graph, u: Int) : ListBuffer [TLabel] = 
     {
         var profileList = ListBuffer [TLabel] ()
...
object GraphqlOptTest extends App
{
    ...
    for (i <- 0 until 5) { 
        ...
        for (mat <- matcher.getMatches ) println (stringOf (mat))   // print Matches
    } // for 
...
object GraphqlOptTest2 extends App
{
    ...
    for (i <- 0 until 5) {
        time { matcher.bijections () }                        // time the matcher
        println ("Match Count: " + matcher.getMatches.size)
        for (mat <- matcher.getMatches ) println (stringOf (mat))
    } // for
