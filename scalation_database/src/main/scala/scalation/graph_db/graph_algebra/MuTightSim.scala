...
import scala.collection.mutable.{ArrayStack, ListBuffer, Map, HashMap}
      ...
      extends GraphMatcher (g, q)
{
    ...
    private val listOfMatchedBallVertices = ListBuffer [Int] ()              // contains list of center vertices
    ...
    { 
         ...
         for ((c, phi_c) <- matches) {
             println (s"(c, phi_c) = ($c, ${stringOf(phi_c)})")
    ...
    def tightSim (phi: Array [SET [Int]]): HashMap [Int, Array [SET [Int]]] =
    {
        ...
        for (center <- gCenters) {                                      // for each mapped data graph center
            ...
            println (s"center = $center, mat = ${stringOf(mat)}")
