    ...
    */
    ...
    override def equals (g2_ : Any): Boolean =
    {
        ...
        case _: MuGraph [TLabel] =>
                ...
                if (stringOf(ch) != stringOf(g2.ch)) { println ("failed on ch"); return false }
                if (stringOf(id) != stringOf(g2.id)) { println ("failed on id"); return false }
                if (stringOf(label) != stringOf(g2.label)) { println("failed on label"); return false }
