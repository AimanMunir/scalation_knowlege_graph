...
import scala.collection.mutable.{ArrayStack, HashMap, ListBuffer, Map}
      ...
      extends GraphMatcher (g, q)
{
    ...
    if (DEBUG) println (s"phi0 = ${stringOf(phi0)}")
    ...
    def refine (phi: Array [SET [Int]]): Map [Int, Array [SET [Int]]] =
    {
        ...
        if (DEBUG) println (s"phi = ${stringOf(phi)}")
        ...
        for (center <- gCenters) {                                      // for each mapped data graph center
            ...
            if (DEBUG) println (s"center = $center, mat = ${stringOf(mat)}")
            balls += center -> ball
