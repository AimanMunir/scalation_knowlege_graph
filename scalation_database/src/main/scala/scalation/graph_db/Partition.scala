...
class Partition [TLabel: ClassTag] (g: Graph [TLabel])
{
    ...
    private val ilabel   = Array.range (0, g.size)               // current integer label for each vertex
    private val ilabel2  = Array.ofDim [Int] (g.size)            // new integer label for each vertex
    private val pop      = Array.ofDim [Int] (g.size)            // holder for population/frequency
    ...
    def group_ord (k: Int)
    {
        val gsize = ceil (ilabel.length / k.toDouble).toInt
    ...
    def group_lp (k: Int)
    {
        ...
        for (it <- 1 to MAX_ITER) {
            ...
            for (i <- ilabel.indices) {
                ...
                nDistinct = ilabel.distinct.length

                if (ilabel.distinct.length <= k) return                 // quit when only k parts
            } // for
    ...
    def partition ()
    {
        ...
        var part = 0
        for (i <- 0 until 10) { // ilabel.length) {                           // put vertex into map based on ilabel
...
//          val gi = new Graph [TLabel] (ch2, lab2, g.inverse, g.name + "_" + i, vid2)
    ...
    {
        val child  = g.ch(i).toArray.map (ilabel(_))             // the labels of the children of vertex i
        val parent = g.pa(i).toArray.map (ilabel(_))             // the labels of the parents of vertex i
